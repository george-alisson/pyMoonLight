...
VKeyboardRenderer.DEFAULT = VKeyboardRenderer(None,
    (255, 255, 255),
    ((255, 255, 255), (0, 0, 0)),
    ((0, 0, 0), (255, 255, 255)),
    ((180, 180, 180), (0, 0, 0)),
...
def synchronizeLayout(primary, secondary, surface_size):
    ...
    if (primary.key_size < secondary.key_size):
        # LOGGER.warning('Normalizing key size from secondary to primary')
        secondary.key_size = primary.key_size
    elif (primary.key_size > secondary.key_size):
        # LOGGER.warning('Normalizing key size from primary to secondary')
        primary.key_size = secondary.key_size
    if (primary.size[1] > secondary.size[1]):
        # LOGGER.warning('Normalizing layout size from secondary to primary')
        secondary.set_size(primary.size, surface_size)
    elif (primary.size[1] < secondary.size[1]):
        # LOGGER.warning('Normalizing layout size from primary to secondary')
